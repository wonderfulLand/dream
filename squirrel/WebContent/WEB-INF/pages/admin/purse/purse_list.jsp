			...
			<form class="form-horizontal" id="myserchform" name="myform"
				...
				<div class="form-group">
					<div class="col-sm-5" style="text-align: center;">
						...
						<input type="number" placeholder="请输用户id" name="userId"
							value="${searchpurse.userId}" class="form-control" />
					</div>
					<div class="col-sm-5" style="text-align: left;">
						<select name="state" data-style="btn-info" class="form-control"
							id="myselected">
				...
				<tbody>
					<c:forEach items="${purseGrid.rows}" var="item">
						<tr>
							...
							<td><c:if test="${item.state==null}">
									<span>-</span>
								</c:if> <c:if test="${item.state==0}">
									<span class="btn-primary btn-info">尚待审核</span>
								</c:if> <c:if test="${item.state==1}">
									<span class="btn-primary btn-danger">审核失败</span>
								</c:if> <c:if test="${item.state==2}">
									<span class="btn-primary  btn-success">审核成功</span>
								</c:if></td>
							<td><c:choose>
									...
									<c:when test="${item.state==0}">
										<button type="button" class="btn btn-info"
											onclick="doEdit(${item.id})">立即审核</button>
									</c:when>
									<c:otherwise>
				...
				<div class="pagination">
					<ul>
						...
						<c:if test="${purseGrid.current ne 1 }">
							<li><a
								href="<%=basePath%>admin/purseList?pageNum=${purseGrid.current-1 }">上一页</a>
							</li>
						</c:if>
				...
				<div class="modal-body" style="height: 0 auto;">
					<form class="form-horizontal" id="myeditform" name="myform">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" readonly class="form-control" name="recharge"
									style="margin-top: 8px;" />
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" readonly class="form-control"
									name="withdrawals" style="margin-top: 8px;" /> <input
									type="hidden" class="form-control" name="state"
									style="margin-top: 8px;" />
							</div>
						</div>
					</form>
				...
				<div class="modal-footer">
					<div style="text-align: center">
						...
						<button type="button" class="btn btn-success col-sm-4"
							onclick="doSavePass()">通过</button>
						<button type="button" class="btn btn-danger col-sm-4"
							onclick="doSaveRefuse()">拒绝</button>
					</div>
				</div>
			</div>
			<!-- /.modal-content -->
